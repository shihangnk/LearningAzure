...
module IntegersAndNumbers = 
    ...
    printfn "sample int %i" sampleInteger

    /// This is a sample floating point number.
    let sampleDouble = 4.1
    printfn "sampleDouble %4.2f" sampleDouble

    /// This computed a new number by some arithmetic.  Numeric types are converted using
    /// functions 'int', 'double' and so on.
    let test = float (sampleInteger/4 + 5 - 7) / float 7
    printfn "sampleInteger2 %f" test
    let sampleInteger2 = (sampleInteger/4 + 5 - 7) * 7 + int sampleDouble
    printfn "sampleInteger2 %d" sampleInteger2

    /// This is a list of the numbers from 0 to 99.
    let sampleNumbers = [ 0 .. 99 ]
    printfn "The array is %A" sampleNumbers
...
//    printfn "resule %d %f" (sampleFunction1 1 ) (sampleFunction1 5.0)

    let floatFun x = x*x + 4.0
    printfn "test floatFun %f" (floatFun 5.0)

    let multipleFun x y = x * y
//    printfn "multipleFun %f %d" (multipleFun 4.0 5.0) (multipleFun 4 5)  
...
module Booleans =
    ...
    printfn "boolean1 %A" boolean1
    printfn "boolean1 %A" (boolean1 && boolean2)
    printfn "boolean1 %A" (boolean1 || boolean2)
...
module StringManipulation = 
    ...
    let substring = helloWorld.[3..6]
    printfn "%s" substring

    let str = "1234567890abcdefg"
    printfn "sub string %s" str.[3..9]
...
module Lists =
    ...
    let myList = [ 3..10]
    printfn "the list is %A" (myList|>List.take 3)
    ...
    printfn "balcksquares is %A" blackSquares
    ...
    let mutable myNumber = [1..10]
    let filteredNumber =
        myNumber |> List.filter(fun x-> x%5=0) |> List.map(fun x->x+1)
    printfn "filteredNumber is %A" filteredNumber

    myNumber <- List.append myNumber [800]
    printfn "mynumber is %A" myNumber

    let myList1 = [3;4]
    let myList2 = 9 :: myNumber
    printfn "myList2 %A" myList2

    printfn "test @ %A" (myList1 @ myList2)

    let mutable myList3 = [1..5]
    printfn "head %A" myList3.Head
    printfn "Empty %A" myList3.IsEmpty
    printfn "item 3 %A" (myList3.Item(2))
    printfn "length %A" myList3.Length
    printfn "tail %A" myList3.Tail
    ...
    printfn "sum %A" (sum [1..5])


    let list11 = [ 1; 2; 3 ]

    // Properties
    printfn "list1.IsEmpty is %b" (list11.IsEmpty)
    printfn "list1.Length is %d" (list11.Length)
    printfn "list1.Head is %d" (list11.Head)
    printfn "list1.Tail.Head is %d" (list11.Tail.Head)
    printfn "list1.Tail.Tail.Head is %d" (list11.Tail.Tail.Head)
    printfn "list1.Item(1) is %d" (list11.Item(1))
...
module Sequences = 
    ...
    printfn "%A" numbersSeq

    let mySeq = seq { for i in 1..10 -> i*i}
    printfn "%A" mySeq

    let myArray = Seq.toArray mySeq
    printfn "%A" myArray

    
//    let mySeqTable = seq {for i in 1..10 -> (i, i*i)}
//    printfn "%A" mySeqTable
//    let convertSeqTab seqTab =
//        for (x,y) in mySeq do
//            (x, ""+y)
...
module RecordTypes = 
    ...
    let contact2 = 
        { ContactCard.Name = "Alf" 
          Phone = "(206) 555-0157" 
          Verified = false }

    let contact3 = {contact2 with Name="hello"}
    ...
    //printfn "contact 1 %A" contact1
    //printfn "contact 2 %A" contact2
...
//    printfn "Alf's Contact Card: %s" (showContactCard contact1)
    ...
    type myType = 
        ...
        member this.output =
            sprintf "%s %i" this.Col1 this.Col2

    let myObj = {
        Col1 = "hello"
        Col2 = 3
    }
    printfn "%s" myObj.output
